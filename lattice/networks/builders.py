...
from .language_models import make_mini_gpt
...
def make_cnn_mlp(
    ...
    num_convs = len(conv_channels)

    cnn = CNN(
        input_shape=input_shape,
        feature_dim=cnn_feature_dim,
        channels=conv_channels,
        kernels=[kernel_size] * num_convs,
        strides=[stride] * num_convs,
        paddings=[padding] * num_convs,
        activation=activation,
    )
    head = make_mlp(
...
def build_model(config: Dict) -> nn.Module:
    ...
    raise ValueError(
        ...
        "['mlp', 'mlp_classifier', 'cnn_mlp', 'rnn', 'transformer', 'mini_gpt', 'sequential']"
    )
